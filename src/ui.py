...
try:
    ...
    SYSTEM_INSTRUCTIONS = (
        "You are a logistics assistant. Always use query_telemetry_db, "
        "fetch_corridor_conditions, and search_compliance_sop before answering."
    )
...
# Seed system prompt once per session thread (CLI does this; UI previously skipped it)
            ...
            if "traces" in entry:
                for trace in entry["traces"]:
                    if trace["type"] == "tool_input":
                        ...
                        if trace["name"] == "query_telemetry_db" and isinstance(trace.get("args"), dict):
                            sql_text = trace["args"].get("sql_query", "")
                            if sql_text:
    ...
    if user_input := st.chat_input("Query fleet telemetry, corridor updates, or compliance thresholds..."):
        ...
        with st.chat_message("assistant", avatar="🤖"):
            ...
            with st.status("🧠 Initializing Core Reasoner Node...", expanded=True) as status:
                        ...
                        if node_name == "reasoner":
                            ...
                            if hasattr(latest_msg, "tool_calls") and latest_msg.tool_calls:
                                ...
                                for tool_call in latest_msg.tool_calls:
                                    ...
                                    # Keep SQL expanded so the generated query is visible
                                    expand_sql = tool_call["name"] == "query_telemetry_db"
                                    with st.expander(
                                        f"📥 View Generated Input ({tool_call['name']})",
                                        expanded=expand_sql,
                                    ):
                                        if expand_sql and isinstance(tool_call.get("args"), dict):
                                            sql_text = tool_call["args"].get("sql_query", "")
                                            if sql_text:
                                                st.code(sql_text, language="sql")
                ...
                status.update(label="Incident Matrix Evaluation Complete", state="complete", expanded=True)
                
            # Re-render traces outside collapsed status so SQL stays visible
            if current_traces:
                st.markdown("### 🔎 Tool Execution Trace")
                for trace in current_traces:
                    if trace["type"] == "tool_input":
                        st.markdown(f"**⚡ Tool call:** `{trace['name']}`")
                        if trace["name"] == "query_telemetry_db" and isinstance(trace.get("args"), dict):
                            sql_text = trace["args"].get("sql_query", "")
                            if sql_text:
                                st.code(sql_text, language="sql")
                        with st.expander(f"📥 Args JSON ({trace['name']})", expanded=False):
                            st.json(trace["args"])
                    elif trace["type"] == "tool_output":
                        with st.expander(f"📤 Output ({trace['name']})", expanded=trace["name"] == "query_telemetry_db"):
                            st.code(trace["content"], language="text")
            ...
            if final_response:
                ...
                if not current_traces:
                    error_fallback += " No tools were called — check that the system prompt is loaded and the model supports tool calling."
